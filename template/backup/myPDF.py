...
for timestep in universe.trajectory:
    ...
    i=1
    while i < nbins:
        selection = "name ROH and sphlayer "+str(distance[i-1])+" "+str(distance[i])+" (name BB and bynum 1:3672)"
        ...
        count[i] = group.n_residues
        print "Count     : "+str(count[i])
        print "Volume    : "+str(volume[i])
        pdf[i] += count[i]
        i += 1
        print "##################################"
...
np.savetxt(str(title)+"-pdf.dat", (distance, pdf))
